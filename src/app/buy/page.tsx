...
const BuyPage = () => {
  ...
  // useRef para timeouts compatível com browser e node
  const pollTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const isMounted = useRef(true);

  // ref para ter sempre o status atual dentro do closure do poll
  const paymentStatusRef = useRef<PaymentStatus>(paymentStatus);

  // manter o ref sincronizado sempre que o state mudar
  useEffect(() => {
    paymentStatusRef.current = paymentStatus;
  }, [paymentStatus]);

  // helper para manter state + ref sempre sincronizados
  const setPaymentStatusSynced = (
    value: PaymentStatus | ((prev: PaymentStatus) => PaymentStatus)
  ) => {
    if (typeof value === 'function') {
      const next = (value as (p: PaymentStatus) => PaymentStatus)(paymentStatusRef.current);
  ...
  const getSuccessRedirectPath = (productId?: string) => {
    ...
    if (selectedAppId === '100151') return '/success';
      ...
      try {
        ...
        if (storedPaymentData) {
          ...
          if (!parsed.pix?.qrcode_base64 || !parsed.pix?.code || !parsed.external_id) {
            toast({ variant: 'destructive', title: 'Erro', description: 'Dados de pagamento incompletos. Redirecionando para o início.' });
            setIsLoading(false);
          ...
          setIsLoading(false);
          const currentStatus: PaymentStatus = parsed.status ? parsed.status.toUpperCase() as PaymentStatus : 'PENDING';
          setPaymentStatusSynced(currentStatus);
          ...
          const startPolling = (externalId: string) => {
            ...
            const poll = async () => {
              if (!isMounted.current || paymentStatusRef.current === 'PAID' || attempt >= maxAttempts) {
              ...
              try {
                ...
                if (res.status === 404) {
                  // ainda não existe transação criada/visível na API
                  console.log("Transação não encontrada ainda, continuando polling.");
                } else if (!res.ok) {
                  // erro na requisição
                  setPaymentStatusSynced('UNKNOWN');
                  return;
                } else {
                  const statusData = await res.json();
                  const newStatus: PaymentStatus = statusData.status?.toUpperCase() as PaymentStatus || 'PENDING';

                  if (isMounted.current) {
                    if (newStatus !== paymentStatusRef.current) {
                      setPaymentStatusSynced(newStatus);
                    }
                ...
                if (isMounted.current) setPaymentStatusSynced('UNKNOWN');
                return;
              }
          ...
          if (parsed.created_at) {
            ...
            timerId = setInterval(() => {
              ...
              if (newTimeLeft <= 0) {
                if (timerId) clearInterval(timerId);
                setPaymentStatusSynced(prev => prev === 'PENDING' ? 'EXPIRED' : prev);
              }
            }, 1000);
          }
          ...
          toast({ variant: 'destructive', title: 'Erro', description: 'Não foi possível carregar os dados do pagamento. Por favor, tente novamente.' });
          setIsLoading(false);
          setTimeout(() => router.push('/'), 2000);
        }
      } catch (error) {
        toast({ variant: 'destructive', title: 'Erro', description: 'Ocorreu um erro ao carregar os dados do pagamento. Redirecionando.' });
        setIsLoading(false);
        setTimeout(() => router.push('/'), 2000);
      }
    };
  ...
  useEffect(() => {
    ...
    if (paymentStatus === 'PAID') {
      ...
      if (pollTimeoutRef.current) clearTimeout(pollTimeoutRef.current);
      toast({ variant: "destructive", title: "Pagamento Expirado/Cancelado", description: "Por favor, inicie uma nova compra." });
      localStorage.removeItem('paymentData');
        ...
        <div className="flex flex-col md:mx-auto md:my-6 md:max-w-[600px] md:rounded-2xl md:bg-gray-50 overflow-hidden">
          ...
          <div className="flex flex-col gap-6 px-4 pb-8 pt-5 md:p-10 md:pt-6">
            ...
            {paymentStatus === 'UNKNOWN' && (
              <Button onClick={() => router.refresh()} className="mt-8" variant="outline">
